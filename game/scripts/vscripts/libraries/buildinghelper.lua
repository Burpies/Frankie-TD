...
-- sell building function inspired by Arne
function BuildingHelper:SellBuilding(keys)
    local building = keys.unit
    local hero = building:GetOwner()
    local refund_factor = 0.75

    --BuildingHelper:print("SellBuilding "..building:GetUnitName().." "..building:GetEntityIndex())

    -- Refund
    if building:GetUnitName() == "tower_cannon" or building:GetUnitName() == "tower" or building:GetUnitName() == "tower_tier2" or building:GetUnitName() == "tower_cannon2" then
        refund_factor = 1.00
    else
        refund_factor = 0.75
    end

    local bounty_gold_cost = math.floor(building:GetMaximumGoldBounty() * refund_factor)

    hero:ModifyGold(bounty_gold_cost, true, 0)
    PopupGoldGain(building, bounty_gold_cost)
