...
function LightningJump(keys)
	...
	if ability.current_jump[ability.instance] ~= nil then
		damage_table.damage = damage*((1-damage_percent_add)^(ability.current_jump[ability.instance] -1))
	else
    ...
    function()
		...
		ability.current_jump[current] = ability.current_jump[current] + 1
...
function NewInstance(keys)
	...
	-- Count up for damage increase/decrement purposes
	ability.current_jump[ability.instance] = 1
